...
let books = [{id: 1 , isbn:  1234 , title : "this is a title of the book" , summary : "this is a loooooooooooooooong summary of the book"} , 
{id: 2 , isbn:  1235 , title : "this is a title of the book" , summary : "this is a loooooooooooooooong summary of the book"} ,
{id: 3 , isbn:  1236 , title : "this is a title of the book" , summary : "this is a loooooooooooooooong summary of the book"}]

//generating random number to get random quote

let ran = Math.floor(Math.random() * 172);

//getting books table from database
...
// creating an empty array to pass it to selected when visiting the home page
let selected = [{
    title : "" ,
    notes : ""
}]
...
app.get("/" , async (req , res) => {
    await getBooks();
    res.render("index.ejs" , {books : books ,  selected : selected[0] , notes : `"${quotes[ran].quote}"` , author : "- " +  quotes[ran].author});
})



app.post("/book-text" , async (req , res) => {
    const isbn = req.body.bookId;
    await getBooks();
    //isbn is a string
    const result = await db.query(`SELECT * FROM books WHERE isbn = '${isbn}'`);
    let selectedBook = result.rows;
    // the result.rows returns an array with only one element so access it by selected[0];
    res.render("index.ejs" , {selected : selectedBook[0] , books : books , notes : "Notes" , author : ""});
})

app.post("/book-image" , async (req , res) => {
    const isbn = req.body.bookId;
    await getBooks();
    const result = await db.query(`SELECT * FROM books WHERE isbn = '${isbn}'`);
    let selectedBook = result.rows;
...
app.post("/add-book" , async (req , res) => {
    ...
    const result = await db.query("INSERT INTO books (title , isbn , summary , date , notes , rating) VALUES ($1 , $2 , $3 , $4 , $5 , $6) RETURNING *", [title, isbn , summary , date , notes , rating]);
    await getBooks();
...
app.post("/delete" , async (req ,res) => {
    const isbn = req.body.dltHidden;
    const result = await db.query("DELETE FROM books WHERE isbn = $1 RETURNING *" , [isbn]);
...
app.post("/sort-books", async (req, res) => {
    ...
    switch (option) {
        ...
        case "ratingLowToHigh":
            const result1 = await db.query("SELECT * FROM books ORDER BY rating ASC");
            books = result1.rows;
            break;
        case "recencyOldToNew":
            const result2 = await db.query("SELECT * FROM books ORDER BY date ASC");
            books = result2.rows;
            break;
        case "recencyNewToOld":
            const result3 = await db.query("SELECT * FROM books ORDER BY date DESC");
            books = result3.rows;
            break;
        default:
            // Random way of getting books
            break;
    }
    ...
    res.render("index.ejs", {
        books: books,
        selected: selected[0],
        notes: `"${quotes[ran].quote}"`,
        author: "- " + quotes[ran].author
    });
});


app.listen(port , (req , res) => {
    console.log("succsesfully listening on port "  + port);
})
